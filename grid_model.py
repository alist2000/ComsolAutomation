...
# --- new: Updated output paths for v5.1 ---
OUTPUT_DIR = Path("results_v5.1")
OUTPUT_DIR.mkdir(exist_ok=True)
DB_PATH = OUTPUT_DIR / "simulation_results_v5.1.db"

# --- new: Material properties now stored as a single dict for easy saving ---
MATERIALS = {
...
# ── DB helpers (updated for v5.1 schema) ──────────────────────────

def _setup_db(db_file: Path) -> None:
    """Initializes the database with the new v5.1 schema."""
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()
    # --- new: Added atlas_path and materials_json to simulations table ---
    cur.execute("""CREATE TABLE IF NOT EXISTS simulations (
                     ...
                     atlas_path     TEXT,
                     materials_json TEXT)""")
    # --- new: Added modes_path to eigenfrequencies table ---
...
def _update_run_with_paths(db_file: Path, run_id: int, atlas_path: str, materials_json: str):
...
def _log_freqs_and_modes(db_file: Path, run_id: int, kx: float, ky: float,
                         freqs: List[float], modes_path: str):
    """Logs frequencies and the path to the corresponding mode shape file."""
    conn = sqlite3.connect(db_file)
    cur = conn.cursor()
    cur.executemany("""INSERT INTO eigenfrequencies
                       (run_id, kx, ky, mode_number, frequency_hz, modes_path)
                       VALUES (?,?,?,?,?,?)""",
                    [(run_id, kx, ky, i + 1, f, modes_path) for i, f in enumerate(freqs)])
    conn.commit()
    conn.close()
...
# ── Worker (updated with corrected evaluation call) ────────────────
...
def _build_and_solve(seed: int):
    ...
    try:
        ...
        model.parameter("a", f"{A}[m]"); model.parameter("kx", "0"); model.parameter("ky", "0")

        comp = model.java.component().create("comp1", True)
        geom = comp.geom().create("geom1", 2)
        cell = A / GRID_SIZE; start = -A / 2
        ...
        pbc_x.set("PeriodicType", "Floquet"); pbc_x.set("kFloquet", ["kx", "0", "0"])
        pbc_y = solid.create("pbc_y", "PeriodicCondition", 1)
        pbc_y.selection().set(pbc_y_nodes)
        pbc_y.set("PeriodicType", "Floquet"); pbc_y.set("kFloquet", ["0", "ky", "0"])
        ...
        eig.set("neigsactive", "on"); eig.set("neigs", str(N_EIG))
        eig.set("shiftactive", "on"); eig.set("shift", "0")
        ...
        for kx, ky in k_points:
            ...
            # === PATCHED SECTION ============================================
            modes_path = f"modes_run_{run_id}_kx_{kx:.2f}_ky_{ky:.2f}.npz"
            # No need to specify dataset; mph finds the default solution.
            u_modes = model.evaluate('u')
            v_modes = model.evaluate('v')
            np.savez_compressed(OUTPUT_DIR / modes_path, u=u_modes, v=v_modes)
            # ================================================================

            _log_freqs_and_modes(DB_PATH, run_id, kx, ky, freqs, modes_path)

            # --- save the full model so validators can reuse the mesh ---
            # save_path = OUTPUT_DIR / f"grid32_seed_{seed}.mph"
